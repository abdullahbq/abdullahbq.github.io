class FileViewer extends HTMLElement {
    constructor() {
        super();
        this.data = {};
        this.debouncedResize = this.debounce(() => this.responsiveSidebar(), 200); // Debounce resize event
    }

    connectedCallback() {
        this.setupSidebar();
        window.addEventListener('load', () => this.responsiveSidebar());
        window.addEventListener('resize', this.debouncedResize); // Use debounced resize event
    }
    ...
    renderFolder(folder, folderName) {
        ...
        const filesHTML = folder.map((fileName, fileIndex) => `
            <li id="${fileIndex}" data-file="${fileIndex}" data-folder="${folderName}" class="file mb-2" style="cursor: pointer">
    ...
    render() {
        ...
        this.innerHTML = `<section class="courses-section">                         
                ...
                ${Object.keys(this.data).map((folderName, folderIndex) => `
                    <div id="${folderName}">
                        <button
                            ...
                            class="w-100 h-20 d-flex align-items-center border-1 my-1"
                            style="font-size: 1.05rem; font-weight: 600; height: 38px"
                            data-bs-toggle="collapse"
                            data-bs-target="#folder${folderIndex}">
